...
onmessage = function (e) {
 ...
 console.log(`Search query: ${query}, options: ${JSON.stringify(opts)}`);
  ...
  for (let verse of kjvStrongsData.verses) {
    if (verse.text && strongsRe.test(verse.text)) {
      ...
      if (strongMatch) {
		...
		if (!verse.text) {
		  console.log(`Missing text for ${verse.book_name} ${verse.chapter}:${verse.verse}`);
		  continue;  // Skip verses with missing text
		}
